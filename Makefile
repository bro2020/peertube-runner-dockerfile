...
build: ## Build peertube-runner image
	docker buildx build --build-arg DOCKER_USER=$(DOCKER_USER) --target runner -t peertube-runner:latest -f Dockerfile-ffmpeg-rockchip .
.PHONY: build

build-whisper_ctranslate2: ## Build peertube-runner image with whisper-ctranslate2
	docker buildx build --build-arg DOCKER_USER=$(DOCKER_USER) --target whisper_ctranslate2 -t peertube-runner:latest-whisper_ctranslate2 -f Dockerfile-ffmpeg-rockchip .
